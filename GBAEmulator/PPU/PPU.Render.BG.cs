...
namespace GBAEmulator
{
    partial class PPU
    {
        ...
        */
        ...
        private static void FillBGWindow(ref bool[] BGWindow, bool value)
        {
            if (value) for (int x = 0; x < width; x++) BGWindow[x] = true;
            else BGWindow = new bool[width];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private void CreateWindow(ref bool[] Window, bool WindowInEnable, byte X1, byte X2, byte Y1, byte Y2)
        {
            if (Y1 <= Y2)
            {
                // no vert wrap
        ...
        {
            FillBGWindow(ref Window, WinOut);  // clear to WINOUT
            ...
            for (byte window = 1; window <= 1; window--)
            {
                ...
                this.CreateWindow(ref Window, (window == 0) ? Win0In : Win1In,
                    X1, X2, Y1, Y2);
            }
        }
        ...
        private void Render4bpp(ref ushort[] Line, ref bool[] Window, int StartX, int XSign, uint TileLineBaseAddress,
            ...
            for (int dx = 0; dx < 4; dx++)  // we need to look at nibbles here
            {
                ...
                if (0 <= ScreenX && ScreenX < width)  // ScreenX is a byte, so always greater than 0
                {
                    if (Line[ScreenX] == 0x8000)
                    {
                        ...
                        if (PaletteNibble > 0 && Window[ScreenX])  // non-transparent
        ...
        private void Render8bpp(ref ushort[] Line, ref bool[] Window, 
            int StartX, int XSign, uint TileLineBaseAddress, bool Mosaic, byte MosaicHSize)
        {
                                            ...
                                            byte CharBaseBlock, bool ColorMode, bool Mosaic, byte MosaicHSize)  // based on y = scanline
        {
            ...
            if (!ColorMode)  // 4bpp
            {
                ...
                this.Render4bpp(ref Line, ref Window, StartX, XSign, Address, (uint)(PaletteBank * 0x20), Mosaic, MosaicHSize);
            }
            else             // 8bpp
            {
                ...
                this.Render8bpp(ref Line, ref Window, StartX, XSign, Address, Mosaic, MosaicHSize);
            }
        }
