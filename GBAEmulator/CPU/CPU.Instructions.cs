using System;

namespace GBAEmulator.CPU
{
    partial class CPU
    {
        private bool Condition(byte field)
        {
            switch (field)
            {
                case 0b0000:  // EQ
                    return Z == 1;
                case 0b0001:  // NE
                    return Z == 0;
                case 0b0010:  // CS
                    return C == 1;
                case 0b0011:  // CC
                    return C == 0;
                case 0b0100:  // MI
                    return N == 1;
                case 0b0101:  // PL
                    return N == 0;
                case 0b0110:  // VS
                    return V == 1;
                case 0b0111:  // VC
                    return V == 0;
                case 0b1000:  // HI
                    return (C == 1) && (Z == 0);
                case 0b1001:  // LS
                    return (C == 0) && (Z == 1);
                case 0b1010:  // GE
                    return N == V;
                case 0b1011:  // LT
                    return N != V;
                case 0b1100:  // GT
                    return (Z == 0) && (N == V);
                case 0b1101:  // LE
                    return (Z == 1) && (N != V);
                case 0b1110:  // AL
                    return true;
                default:
                    throw new Exception(string.Format("Condition field {0:b4} reserved/invalid", field));
            }
        }
        private void Execute(uint instruction)
        {
            if (!Condition((byte)((instruction & 0xf000_0000) >> 28)))
            {
                return;
            }

            switch ((instruction & 0x0e00_0000) >> 25)
            {
                case 0b000:
                    // Multiply / Multiply Long / Single Data Swap / Branch & Exchange / Halfword Data Transfer
                    return;

                case 0b001:
                    // (Data Processing / PSR Transfer)
                    return;

                case 0b010:
                case 0b011:
                    // Single data transfer / Undefined
                    return;

                case 0b100:
                    // Block Data Transfer
                    return;

                case 0b101:
                    // Branch
                    return;

                case 0b110:
                    // Coprocessor Data Transfer
                    return;

                case 0b111:
                    // Coprocessor Data Operation / Coprocessor Register Transfer / SWI
                    return;
            }
        }
    }
}
